...
def main():
    ...
    parser.add_argument('--dataset', default='cifar10', type=str,
                        choices=['cifar10', 'cifar100', 'ucifar100', 'cifar9', 'mnist', 'stl9', 'cs', 'sc', 'ms'],
                        help='dataset name')
    ...
    if args.dataset == 'cifar10' or args.dataset == 'ms' or args.dataset == 'mnist':
        args.num_classes = 10
        if args.arch == 'wideresnet':
            args.model_depth = 28
            args.model_width = 2
    ...
    elif args.dataset == 'cifar9' or args.dataset == 'stl9' or args.dataset == 'cs' or args.dataset == 'sc':
        args.num_classes = 9
        if args.arch == 'wideresnet':
            args.model_depth = 28
            args.model_width = 2
    ...
    labeled_dataset, unlabeled_dataset, test_dataset, test_src_dataset = DATASET_GETTERS[args.dataset](
        args, './data')
    ...
    #checkpoint = torch.load("results/sc@" + str(args.num_labeled) + "/checkpoint.pth.tar")
...
def train(args, labeled_trainloader, unlabeled_trainloader, test_loader, test_src_loader,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for batch_idx in range(args.eval_step):
            ...
            if not args.no_progress:
                if (batch_idx+1) % 32 == 0:
                    #p_bar.set_description("Trin epoch: {}/{}, iter: {}/{}, Loss: {}, Loss_x: {}, Loss_u: {}".format(
                    #    epoch + 1, args.epochs, batch_idx+1, args.eval_step, losses.avg, losses_x.avg, losses_u.avg
                    #))
                    #p_bar.update()
                    print("Train epoch: {}, iter: {}/{}, Loss: {}, Loss_x: {}, Loss_u: {}".format(
                        epoch + 1, batch_idx+1, args.eval_step, losses.avg, losses_x.avg, losses_u.avg
                        ))
                """
                p_bar.set_description("Train Epoch: {epoch}/{epochs:4}. Iter: {batch:4}/{iter:4}. Data: {data:.3f}s. Batch: {bt:.3f}s. Loss: {loss:.4f}. Loss_x: {loss_x:.4f}. Loss_u: {loss_u:.4f}. Mask: {mask:.2f}. ".format(
                    epoch=epoch + 1,
                    epochs=args.epochs,
                    batch=batch_idx + 1,
                    iter=args.eval_step,
        ...
        if args.local_rank in [-1, 0]:
            ...
            #test_loss, test_acc = test(args, test_src_loader, test_model, epoch)
